...
async def photo_handler1(update: Update, context: CallbackContext) -> None:
    ...
    if 'waiting_for_text' in user_data:
        ...
        if update.message.photo:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT telegram_id FROM users WHERE role = 1")
...
async def broadcast(update: Update, context: CallbackContext) -> None:
    if not is_admin(update.effective_user.id):  # await для is_admin
        await update.message.reply_text("Вы не имеете прав для использования этой команды.")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE, edit=False) -> int:
    ...
    if is_user_banned(user_id):
        await update.message.reply_text(
            "Вы заблокированы и не можете создать анкету. Бот для вас теперь платный, пишите по разбану сюда")
