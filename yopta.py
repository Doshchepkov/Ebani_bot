...
BANS = [] #Сюда вбиваешь тг айдишники балбесов, я думаю здесь максимум шутк 100 наберется
# UPDATE users SET reports = 0;  (обнулит всем баны)

#DELETE FROM users WHERE telegram_id = %s удалить человека из базы перед баном

#Человек тебе пишет в лс для разбана, ты его отправляешь к боту @userinfo, он ему показывает его id
#и ты можешь человека разбанить, удалив из списка
