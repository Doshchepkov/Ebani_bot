...
async def handle_like_dislike(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if action == 'like':
            ...
            # Проверяем, поставил ли пользователь лайк этому профилю ранее
            cursor.execute("SELECT * FROM likes WHERE liker_id = %s AND liked_id = %s", (user_id, target_id))
            existing_like = cursor.fetchone()

            if existing_like:
                # Проверяем, было ли уведомление отправлено ранее
                cursor.execute("SELECT * FROM users WHERE telegram_id = %s", (user_id,))
            ...
            logger.info(f"Попытка поставить дизлайк: disliker_id={user_id}, disliked_id={target_id}")
            ...
            # Получаем ID пользователя, подающего жалобу
            cursor.execute('SELECT id FROM users WHERE telegram_id = %s', (user_id,))
            dbui = cursor.fetchone()

            if not dbui:
                await query.edit_message_text(text="Не удалось найти ваш профиль.")
                return
            ...
            cursor.execute('SELECT id FROM users WHERE telegram_id = %s', (reported_user_id,))
            dbrepid_result = cursor.fetchone()

            if not dbrepid_result:
                await query.edit_message_text(text="Не удалось найти профиль для жалобы.")
                return

            dbrepid = dbrepid_result[0]

            # Проверяем, была ли уже подана жалоба на этот профиль от этого пользователя
            ...
            if not prev:
                # Если жалоба не была подана ранее, увеличиваем счетчик жалоб и вставляем запись
                cursor.execute("UPDATE users SET reports = reports + 1 WHERE id = %s", (dbrepid,))
                conn.commit()
