...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE, edit=False) -> int:
    ...
    if is_user_banned(user_id):
        await update.message.reply_text("Вы заблокированы и не можете создать анкету. Бот для вас теперь платный, пишите по разбану сюда")
    ...
    try:
        ...
        if action == 'like' and not been:
            ...
            if mutual_like:
                ...
                if user1[11] == 2:  # Предполагается, что роль находится в user[12]
                    total1 += f"\nAdmin★\n"
                # Формирование текста профиля
                total1 += f"У вас взаимный лайк! Вот контакт вашего совпадения: {user1_tag}\n"
                total1 = (
                    f"{user1[1]}, "
                ...
                if user2[11] == 2:  # Предполагается, что роль находится в user[12]
                    total2 += f"\nAdmin★\n"
                total2 += f"У вас взаимный лайк! Вот контакт вашего совпадения: {user2_tag}"
                # Формирование текста профиля
                total2 = (
                    f"{user2[1]}, "
                ...
                await context.bot.send_photo(chat_id=user_id, photo = user2[6],
                                               caption= total2[:1024], reply_markup=keyboard)
                await context.bot.send_photo(chat_id=target_id, photo = user1[6],
                                               caption= total1[:1024], reply_markup=keyboard)
            else:
