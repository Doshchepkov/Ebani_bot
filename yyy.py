...
def get_random_user(update: Update):
    try:
        ...
        # Получаем город пользователя, который инициировал поиск
        user_id = update.effective_user.id
        ...
        if city_result:
            city = city_result[0]
            cursor.execute(
                "SELECT * FROM users WHERE city = %s AND telegram_id != %s AND telegram_id NOT IN (SELECT liked_id FROM likes WHERE liker_id = %s)",
        ...
        # Если не нашлось пользователей в том же городе, ищем по региону
        ...
        if region_result:
            ...
            cursor.execute(
                "SELECT * FROM users WHERE region = %s AND telegram_id != %s AND telegram_id NOT IN (SELECT liked_id FROM likes WHERE liker_id = %s)",
        ...
        # Если не нашлось пользователей ни в городе, ни в регионе, ищем по всей базе
        cursor.execute(
            "SELECT * FROM users WHERE telegram_id != %s AND telegram_id NOT IN (SELECT liked_id FROM likes WHERE liker_id = %s)",
            (user_id, user_id)
        )
        ...
        if users_any:
            return random.choice(users_any)
        else:
            return None
